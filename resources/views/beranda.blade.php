...
<div class="log-history-container">
    ...
    @else
        <div class="log-feed">
            ...
            @foreach ($logs as $log)
                @php
                    // Mencegah duplikat log berdasarkan waktu created_at
                    if (in_array($log->created_at, $seenCreatedAt)) {
                        continue;
                    }
                    $seenCreatedAt[] = $log->created_at;

                    $oldData = is_string($log->old_data) ? json_decode($log->old_data, true) : (is_array($log->old_data) ? $log->old_data : []);
                    $newData = is_string($log->new_data) ? json_decode($log->new_data, true) : (is_array($log->new_data) ? $log->new_data : []);
                    $changes = [];

                    // Menelusuri perubahan antara data lama dan baru
                    foreach ($oldData as $key => $value) {
                        if (isset($newData[$key]) && $value != $newData[$key]) {
                            if ($key === 'lamp_foto_karyawan') {
                                $changes[] = "<strong>Foto Karyawan:</strong><br>
                                    <div class='photo-comparison'>
                                        <img src='/storage/" . ($oldData['lamp_foto_karyawan'] ?? 'default-photo.jpg') . "' alt='Foto Sebelum' class='employee-photo'>
                                        <span>→</span>
                                        <img src='/storage/" . ($newData['lamp_foto_karyawan'] ?? 'default-photo.jpg') . "' alt='Foto Setelah' class='employee-photo'>
                                    </div>";
                            } else {
                                $changes[] = "<strong>" . ucfirst(str_replace('_', ' ', $key)) . "</strong>: <span class='old-value'>{$value}</span> → <span class='new-value'>{$newData[$key]}</span>";
                            }
                        }
                    }

                    // Menampilkan lampiran lainnya
                    foreach (['lamp_ktp', 'lamp_sk_kartap', 'lamp_sk_promut', 'lamp_kontrak', 'lamp_buku_nikah', 'lamp_kk', 'lamp_ktp_pasangan', 'lamp_akta_1', 'lamp_akta_2', 'lamp_akta_3', 'lamp_bpjs_kes', 'lamp_bpjs_tk', 'lamp_kartu_npwp', 'lamp_buku_rekening', 'avatar_karyawan'] as $attachment) {
                        if (isset($oldData[$attachment]) && isset($newData[$attachment]) && $oldData[$attachment] != $newData[$attachment]) {
                            $oldAttachmentUrl = Storage::url($oldData[$attachment]);
                            $newAttachmentUrl = Storage::url($newData[$attachment]);
                            $changes[] = "<strong>" . ucfirst(str_replace('_', ' ', $attachment)) . "</strong>: 
                                <div class='attachment-change'>
                                    <strong>Perubahan:</strong>
                                    @if (in_array(pathinfo($newAttachmentUrl, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif']))
                                        <img src='{$newAttachmentUrl}' alt='{$attachment}' class='attachment-image' />
                                    @elseif (in_array(pathinfo($newAttachmentUrl, PATHINFO_EXTENSION), ['pdf', 'docx', 'xlsx']))
                                        <a href='{$newAttachmentUrl}' target='_blank' class='attachment-link'>Lihat File</a>
                                    @else
                                        <span>{$newData[$attachment]}</span>
                                    @endif
                                </div>";
                        }
                    }

                    // Logika Like dan Foto Profil
                    $likeCount = $log->likes->count();
                    $userLiked = $log->likes->where('user_id', Auth::id())->isNotEmpty();
                    $isAdmin = Auth::user()->role === 'admin';

                    // Mengambil foto profil dari data pegawai
                    $profilePhotoUrlBefore = $log->dataPegawai->profile_photo_url
                        ? '/storage/' . $log->dataPegawai->profile_photo_url
                        : '/storage/default-profile.jpg';

                    $profilePhotoUrlAfter = isset($newData['profile_photo_url']) && $newData['profile_photo_url']
                        ? '/storage/' . $newData['profile_photo_url']
                        : $profilePhotoUrlBefore;

                    // Cek perubahan foto profil
                    $isProfilePhotoChanged = isset($newData['profile_photo_url']) && $newData['profile_photo_url'] !== $log->dataPegawai->profile_photo_url;

                    // Fitur Tambahan: Ambil status pegawai dan jabatan
                    $statusPegawai = $log->dataPegawai->status ?? 'Tidak Diketahui';
                    $jabatanPegawai = $log->dataPegawai->jabatan ?? 'Tidak Diketahui';
                    $nikPegawai = $log->dataPegawai->nik ?? 'Tidak Diketahui';
                    $divisiPegawai = $log->dataPegawai->divisi ?? 'Tidak Diketahui';
                @endphp
                ...
                <div class="log-card">
                    ...
                    <div class="log-body">
                        ...
                        @if ($isProfilePhotoChanged)
                            <div class="profile-photo-change">
                                <p><strong>Foto Profil Sebelumnya:</strong></p>
                                <a href="{{ $profilePhotoUrlBefore }}" target="_blank">
                                    <img src="{{ $profilePhotoUrlBefore }}" alt="Foto Profil Sebelum" class="profile-photo-before" onerror="this.onerror=null;this.src='/storage/default-profile.jpg';">
                                </a>
                                <p><strong>Foto Profil Setelah:</strong></p>
                                <a href="{{ $profilePhotoUrlAfter }}" target="_blank">
                                    <img src="{{ $profilePhotoUrlAfter }}" alt="Foto Profil Setelah" class="profile-photo-after" onerror="this.onerror=null;this.src='/storage/default-profile.jpg';">
                                </a>
                            </div>
                        @endif
                    ...
                    @if ($likeCount > 0)
                        <div class="like-section">
                            <span class="like-count">{{ $likeCount }} Suka</span>
                            @if ($userLiked)
                                <span class="user-liked">Anda Menyukai</span>
                            @endif
                        </div>
                    @endif
                </div>

                <button class="like-btn {{ $userLiked ? 'liked' : '' }}" data-log-id="{{ $log->id }}">
                    ❤️ <span class="like-count">{{ $likeCount }}</span> {{ $userLiked ? 'Unlike' : 'Like' }}
                </button>

                <!-- @if ($isAdmin)
                    <div class="validation-buttons">
                        <button class="check-btn" data-log-id="{{ $log->id }}">✅ Validasi</button>
                        <button class="cross-btn" data-log-id="{{ $log->id }}">❌ Tolak</button>
                    </div>
                @endif -->
                ...
                <div class="log-status">
                    <p><strong>Status: </strong>
                        ...
                        @elseif ($log->validation_status === 'approved')
                        ...
                        @else
                            <span class="btn btn-outline-info">Tidak Diketahui</span>
                        @endif
                    </p>
                </div>

                @if ($log->validation_status === 'pending' && $isAdmin)
                    <div class="validation-buttons">
                        <button class="validate-btn" data-id="{{ $log->id }}" data-status="approved">✅ Approve</button>
                        <button class="validate-btn" data-id="{{ $log->id }}" data-status="rejected">❌ Reject</button>
                    </div>
                @endif
